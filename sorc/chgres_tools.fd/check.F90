...
! some land from (1). They should match.
 ...
 data varname /'alvsf', 'alvwf', 'alnsf', 'alnwf', 'canopy', &
 ...
 file_ceiling="/gpfs/dell1/stmp/George.Gayno/chgres_ceiling/out.sfc.tile1.nc"
 ...
 do i = 1, idim
   if (nint(slmsk(i,j)) == 1) then
     if(ceiling(land_frac(i,j)) /= 1.0_8) then
       print*,'bad point 1', i,j,slmsk(i,j),ceiling(land_frac(i,j))
       stop
     endif
   endif
   if (ceiling(land_frac(i,j)) == 1.0_8) then
     if (nint(slmsk(i,j)) /= 1) then
       print*,'bad point 2', i,j,slmsk(i,j),ceiling(land_frac(i,j))
       stop
     endif
   endif
 enddo
...
! Check data.
 ...
 do var = 1, num_var
   ...
   if (trim(varname(var)) == 'tsfcl') varname(var) = 'tsea'
   if (trim(varname(var)) == 'zorll') varname(var) = 'zorl'
   error=nf90_inq_varid(ncid_ceil, varname(var), varid_ceil)
 ...
 allocate(dummy_ceil3d(idim,jdim,4))
     ...
     do i = 1, idim
       ...
       if (nint(slmsk(i,j)) == 1) then
         if (dummy_ceil3d(i,j,n) /= dummy_frac3d(i,j,n)) then
           print*,'bad pt ',i,j,n,dummy_ceil3d(i,j,n),dummy_frac3d(i,j,n)
         endif
       endif
