...
masks. Thus, only the positional weights are required for the generation
of warmstarts from the restart files.
...
<tr><td row=1>tripole.mx[destination resolution].Ct.to.[Cu][Cv][Bu].bilinear.nc <td> the ESMF weights for mapping warmstart values on a <br>
