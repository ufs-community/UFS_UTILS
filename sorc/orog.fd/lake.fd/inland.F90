...
PROGRAM inland_mask
  ...
  INTEGER :: max_rd 
  REAL :: cutoff
!  INTEGER, PARAMETER :: max_rd = 10
!  REAL, PARAMETER :: cutoff = 0.95

  LOGICAL, ALLOCATABLE :: done(:,:,:) 

!  CALL getarg(0, arg) ! get the program name
!  IF (iargc() /= 1) THEN
!    PRINT*, 'Usage: ', trim(arg), ' [resolution (48,96, ...)] '  
!    STOP
!  ENDIF
...
SUBROUTINE mark_global_inland(cs_res)
  ...
  inland = 1.0
! first recursively mark all ocean cells
...
#ifdef SIX_PANELS
  DO tile = 1, 5
    CALL mark_ocean_rec(i_ctr, j_ctr, tile)
  ENDDO
  tile = 6
  CALL mark_ocean_rec(1, 1, tile)
#endif
...
RECURSIVE SUBROUTINE mark_tile_inland(i, j, t)
  INTEGER, INTENT(IN) :: i, j, t
  
  IF (done(i,j,t)) RETURN
  IF (i > cs_res .OR. i < 1 .OR. j > cs_res .OR. j < 1) RETURN
  IF (land_frac(i,j,t) < 0.9) THEN
    done(i,j,t) = .true.
    inland(i,j,t) = 0.0
    CALL mark_ocean_rec(i-1, j, t)
    CALL mark_ocean_rec(i+1, j, t)
    CALL mark_ocean_rec(i, j+1, t)
    CALL mark_ocean_rec(i, j-1, t)
  ENDIF
  
END SUBROUTINE mark_tile_inland
