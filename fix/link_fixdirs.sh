...
#             'wcoss2', 'ursa', 'jet', 'orion', 'hercules', 's4', 'gaeac5', 'gaeac6'
...
if [ $# -lt 2 ]; then
    ...
    echo ' Syntax: link_fv3gfs.sh ( nco | emc ) ( wcoss2 |  ursa  | jet | orion | hercules | s4 | gaeac5 | gaeac6 )'
    exit 1
fi
...
if [ $machine != wcoss2 -a $machine != ursa -a $machine != jet -a $machine != orion -a $machine != s4 -a $machine != hercules -a $machine != gaeac5 -a $machine != gaeac6 ]; then
    set +x
