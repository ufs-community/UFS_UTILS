...
export home_dir=$PWD/../..
export APRUN=time
export APRUN_SFC="mpiexec -n 30 -ppn 30"
...
TEST1=$(qsub -V -o $LOG_FILE -e $LOG_FILE -q $QUEUE -A $PROJECT_CODE -l walltime=00:08:00 \
        -N c96.uniform -l select=1:ncpus=60:mem=40GB $PWD/c96.uniform.sh)
...
TEST2=$(qsub -V -o $LOG_FILE -e $LOG_FILE -q $QUEUE -A $PROJECT_CODE -l walltime=00:08:00 \
        -N c96.viirs.vegt -l select=1:ncpus=60:mem=40GB -W depend=afterok:$TEST1 $PWD/c96.viirs.vegt.sh)
...
TEST3=$(qsub -V -o $LOG_FILE -e $LOG_FILE -q $QUEUE -A $PROJECT_CODE -l walltime=00:08:00 \
        -N gfdl.regional -l select=1:ncpus=60:mem=40GB -W depend=afterok:$TEST2 $PWD/gfdl.regional.sh)
...
TEST4=$(qsub -V -o $LOG_FILE -e $LOG_FILE -q $QUEUE -A $PROJECT_CODE -l walltime=00:08:00 \
        -N esg.regional -l select=1:ncpus=60:mem=40GB -W depend=afterok:$TEST3 $PWD/esg.regional.sh)
...
TEST5=$(qsub -V -o $LOG_FILE -e $LOG_FILE -q $QUEUE -A $PROJECT_CODE -l walltime=00:08:00 \
        -N rsg.gsl.gwd -l select=1:ncpus=60:mem=40GB -W depend=afterok:$TEST4 $PWD/regional.gsl.gwd.sh)
...
qsub -V -o ${LOG_FILE} -e ${LOG_FILE} -q $QUEUE -A $PROJECT_CODE -l walltime=00:01:00 \
        -N grid_summary -l select=1:ncpus=2:mem=100MB -W depend=afterok:$TEST5 << EOF
#!/bin/bash
cd ${this_dir}
