...
export OUTDIR="${WORK_DIR:-/work2/noaa/stmp/$LOGNAME}"
...
export OMP_NUM_THREADS=1  # needs to match cpus-per-task
TEST3=$(sbatch --parsable --ntasks-per-node=6 --nodes=2 --mem=75G -t 0:10:00 -A $PROJECT_CODE -q $QUEUE -J c96.fv3.netcdf \
      --open-mode=append -o $LOG_FILE3 -e $LOG_FILE3 ./c96.fv3.netcdf.sh)
...
export OMP_NUM_THREADS=1  # needs to match cpus-per-task
TEST4=$(sbatch --parsable --ntasks-per-node=6 --nodes=1 --mem=75G -t 0:05:00 -A $PROJECT_CODE -q $QUEUE -J c192.gfs.grib2 \
      --open-mode=append -o $LOG_FILE4 -e $LOG_FILE4 ./c192.gfs.grib2.sh)
...
TEST5=$(sbatch --parsable --ntasks-per-node=12 --nodes=1 --mem=75G -t 0:10:00 -A $PROJECT_CODE -q $QUEUE -J 25km.conus.gfs.grib2 \
      --open-mode=append -o $LOG_FILE5 -e $LOG_FILE5 ./25km.conus.gfs.grib2.sh)
...
export OMP_NUM_THREADS=1   # should match cpus-per-task
TEST6=$(sbatch --parsable --ntasks-per-node=6 --nodes=1 --mem=75G -t 0:10:00 -A $PROJECT_CODE -q $QUEUE -J 3km.conus.hrrr.gfssdf.grib2 \
      --open-mode=append -o $LOG_FILE6 -e $LOG_FILE6 ./3km.conus.hrrr.gfssdf.grib2.sh)
...
export OMP_NUM_THREADS=1   # should match cpus-per-task
TEST7=$(sbatch --parsable --ntasks-per-node=6 --nodes=2 --mem=75G -t 0:10:00 -A $PROJECT_CODE -q $QUEUE -J 3km.conus.hrrr.newsfc.grib2 \
      --open-mode=append -o $LOG_FILE7 -e $LOG_FILE7 ./3km.conus.hrrr.newsfc.grib2.sh)
...
export OMP_NUM_THREADS=1   # should match cpus-per-task
TEST8=$(sbatch --parsable --ntasks-per-node=12 --nodes=1 --mem=75G -t 0:10:00 -A $PROJECT_CODE -q $QUEUE -J 13km.conus.nam.grib2 \
      --open-mode=append -o $LOG_FILE8 -e $LOG_FILE8 ./13km.conus.nam.grib2.sh)
...
export OMP_NUM_THREADS=1   # should match cpus-per-task
TEST9=$(sbatch --parsable --ntasks-per-node=12 --nodes=1 --mem=75G -t 0:10:00 -A $PROJECT_CODE -q $QUEUE -J 13km.conus.rap.grib2 \
      --open-mode=append -o $LOG_FILE9 -e $LOG_FILE9 ./13km.conus.rap.grib2.sh)
...
export OMP_NUM_THREADS=1   # should match cpus-per-task
TEST10=$(sbatch --parsable --ntasks-per-node=12 --nodes=1 --mem=75G -t 0:10:00 -A $PROJECT_CODE -q $QUEUE -J 13km.na.gfs.ncei.grib2 \
      --open-mode=append -o $LOG_FILE10 -e $LOG_FILE10 ./13km.na.gfs.ncei.grib2.sh)
...
TEST11=$(sbatch --parsable --ntasks-per-node=12 --nodes=1 --mem=100G -t 0:15:00 -A $PROJECT_CODE -q $QUEUE -J c96.fv3.netcdf2wam \
      --open-mode=append -o $LOG_FILE11 -e $LOG_FILE11 ./c96.fv3.netcdf2wam.sh)
...
export OMP_NUM_THREADS=1   # should match cpus-per-task
TEST12=$(sbatch --parsable --ntasks-per-node=12 --nodes=1 --mem=75G -t 0:10:00 -A $PROJECT_CODE -q $QUEUE -J 25km.conus.gfs.pbgrib2 \
      --open-mode=append -o $LOG_FILE12 -e $LOG_FILE12 ./25km.conus.gfs.pbgrib2.sh)
