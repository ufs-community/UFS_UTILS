...
program ftst_program_setup
  ...
  if (cycle_mon .ne. 10 .or. cycle_day .ne. 3 .or. cycle_hour .ne. 0) stop 3
  if (.not. convert_atm .or. .not. convert_sfc .or. .not. convert_nst) stop 4
  if (regional .ne. 0 .or. halo_bndy .ne. 0 .or. halo_blend .ne. 0) stop 5
  if (trim(mosaic_file_target_grid) .ne. "/scratch1/NCEPDEV/da/George.Gayno/noscrub/reg_tests/chgres_cube/fix/C192/C192_mosaic.nc") stop 6
  if (trim(fix_dir_target_grid) .ne. "/scratch1/NCEPDEV/da/George.Gayno/noscrub/reg_tests/chgres_cube/fix/C192/fix_sfc") stop 7
  if (trim(orog_dir_target_grid) .ne. "/scratch1/NCEPDEV/da/George.Gayno/noscrub/reg_tests/chgres_cube/fix/C192/") stop 8
  if (trim(vcoord_file_target_grid) .ne. "/scratch1/NCEPDEV/da/George.Gayno/ufs_utils.git/UFS_UTILS/reg_tests/chgres_cube/../../fix/fix_am/global_hyblev.l64.txt") stop 9
  if (trim(data_dir_input_grid) .ne. "/scratch1/NCEPDEV/da/George.Gayno/noscrub/reg_tests/chgres_cube/input_data/fv3.history") stop 10
  if (trim(atm_files_input_grid(1)) .ne. 'dynf000.tile1.nc') stop 11
  if (trim(sfc_files_input_grid(1)) .ne. 'phyf000.tile1.nc') stop 12
  if (varmap_file .ne. "NULL") stop 13
  if (thomp_mp_climo_file .ne. "NULL") stop 14
  if (trim(cres_target_grid) .ne. "C192") stop 15
  if (atm_weight_file .ne. "NULL") stop 16
  if (trim(input_type) .ne. "history") stop 17
  if (trim(external_model) .ne. "GFS") stop 18
  if (num_tracers .ne. 3) stop 19
  if (tracers(1) .ne. "sphum" .or. tracers(2) .ne. "liq_wat" .or. tracers(3) .ne. "o3mr") stop 20
  if (tracers_input(1) .ne. "spfh" .or. tracers_input(2) .ne. "clwmr" .or. &
       tracers_input(3) .ne. "o3mr") stop 200
